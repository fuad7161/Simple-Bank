...
import (
	...
	mockdb "github.com/fuad7161/simple-bank/db/mock"
...
func TestGetAccountAPI(t *testing.T) {
	...
	for i := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			tc.buildStubs(store)
			// stat test server and send request
			server := NewServer(store)
			recorder := httptest.NewRecorder()

			url := fmt.Sprintf("/accounts/%d", tc.acountID)
...
func requireBodyMatchAccount(t *testing.T, body *bytes.Buffer, accout db.Account) {
	...
	require.NoError(t, err)
	require.Equal(t, accout, gotAccount)
}
